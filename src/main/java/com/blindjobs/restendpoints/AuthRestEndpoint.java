...
public class AuthRestEndpoint {
    ...
    @Operation(summary = "Check user credential")
    ...
    @PostMapping(value = "api/v1/user/login", produces = "application/json", consumes = "application/json")
    public ResponseEntity<OperationData<?>> loginUser(@RequestBody SingleItemPayload<Login> userPayload) throws Exception {
        return new ResponseEntity<>(authService.checkCredential(userPayload.getData()), HttpStatus.OK);
    }
